...
async def watch_for_button_presses():
    ...
    while True:
        ...
        if device.current_state == SystemState.TAKING_MEASURMENT:
            ...
            await asyncio.sleep(0.5)  # very short delay otherwise
...
async def init_calibration():
    import json
    await asyncio.sleep(1)  # Let everything else settle
    try:
        global calib
        ...
        print(f"Calibration init failed: {e}")
...
async def main():
    # Start all tasks except calibration
    tasks = [
    ...
    # Delay calibration start until system settles
    await asyncio.sleep(1)
    await init_calibration()
